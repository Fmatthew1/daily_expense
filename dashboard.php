...
//Sum by category
$userid=$_SESSION['detsuid'];
	// SQL query to get the total ExpenseCost per categoryName
$sql = "SELECT categoryName, SUM(ExpenseCost) as totalExpense FROM tblexpense  where UserId='$userid' GROUP BY categoryName";
...
$chart_data = array();
// Loop through the query result and populate the chart data array
while ($row = mysqli_fetch_assoc($result)) {
  $chart_data[] = array($row['categoryName'], (int)$row['totalExpense']);
}
...
// Draw the chart
echo "<div id='piechart' style='width: 540px; height: 180px;'></div>";
echo "<script type='text/javascript'>
        ...
        function drawChart() {
          ...
          data.addRows(".json_encode($chart_data).");

		  var options = {
			title: 'Total Expense by Category',
			is3D: true,
			pieSliceText: 'value' // Display actual value of each slice
		  };
